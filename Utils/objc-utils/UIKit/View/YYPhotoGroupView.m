...
//    fromView.hidden = fromViewHidden;
    ...
    if (item.thumbClippedToTop) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:YYPhotoGroupStatusChangedNotification
                                                            object:self
                                                          userInfo:YYPhotoGroupStatusValue(YES)];
...
- (void)dismiss {
    [[NSNotificationCenter defaultCenter] postNotificationName:YYPhotoGroupStatusChangedNotification
                                                        object:self
                                                      userInfo:YYPhotoGroupStatusValue(NO)];
...
- (void)hidePager {
        [UIView animateWithDuration:0.3 delay:2 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveEaseOut animations:^{
            self.pager.alpha = 0;
        }completion:^(BOOL finish) {
        }];
}
